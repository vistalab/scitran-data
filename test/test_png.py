...
class Test_NIMSPNG(object):

    @skipif(not DATADIR)
    def setUp(self):
        # TODO: remake ge_dcm_screenshot with json
        self.ds = nimsdata.parse(os.path.join(DATADIR, 'ge_dcm_sc_screenshot.tgz'), load_data=True, filetype='dicom', ignore_json=True)

    @skipif(not DATADIR)
    def test_write(self):
        with tempfile.TemporaryDirectory() as tempdir:
            outbase = os.path.join(tempdir, 'trashme')
            nimsdata.write(self.ds, self.ds.data, outbase=outbase, filetype='png')
