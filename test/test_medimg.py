...
class test_parse_patient_id(object):
    ...
    def test_parse_valid_patient_id(self):
        patient_id = '1234@scitran/nimsdata'
        s, g, p = nimsdata.medimg.parse_patient_id(patient_id, self.default_subj_code)
        eq_(s, '1234')
        eq_(g, 'scitran')
        eq_(p, 'nimsdata')
...
class test_parse_patient_name(object):
    def test_patient_name_delimiters(self):
        f, l = nimsdata.medimg.parse_patient_name('Last^First')
        eq_((f, l), ('First', 'Last'))
        f, l = nimsdata.medimg.parse_patient_name('First Last')
        eq_((f, l), ('First', 'Last'))
        f, l = nimsdata.medimg.parse_patient_name('FirstLast')
        eq_((f, l), ('', 'Firstlast'))

class test_parse_patient_dob(object):
    def test_parse_valid_patient_dob(self):
        eq_(datetime.datetime(2011, 1, 1, 0, 0),  nimsdata.medimg.parse_patient_dob('20110101'))

    def test_parse_invalid_patient_dob(self):
        eq_(None,  nimsdata.medimg.parse_patient_dob('invalid'))
        eq_(None,  nimsdata.medimg.parse_patient_dob('18990101'))
