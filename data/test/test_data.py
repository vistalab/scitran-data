...
class test_properties_by_type_list(object):
    ...
    def test_fail_module_by_type(self):
        assert_raises(data.DataError, data.data.module_by_type, ('fake', 'type'))

    def test_fail_dict_merge(self):
        assert_raises(data.DataError, data.dict_merge, [], [])

    def test_acquisition(self):
        ok_(data.acquisition_properties_by_type_list(self.type_list))

    def test_session(self):
        ok_(data.session_properties_by_type_list(self.type_list))

    def test_project(self):
        ok_(data.project_properties_by_type_list(self.type_list))
...
class test_parse(object):
    ...
    def test_get_handler(self):
        ...
        assert_raises(data.DataError, data.get_handler, 'fake.fake.fake', READERS)   # doesn't exost
        assert_raises(data.DataError, data.get_handler, 'fake.fake', READERS)        # FIXME
        assert_raises(data.DataError, data.get_handler, 'fake', READERS)             # FIXME

    @skipif(not DATADIR)
    def test_input_not_found(self):
        assert_raises(data.DataError, data.parse, './fake.tgz')

    @skipif(not DATADIR)
    def test_ignore_json_without_filetype(self):
        assert_raises(data.DataError, data.parse, './fake.tgz', ignore_json=True)

    @skipif(not DATADIR)
    def test_no_json_in_tgz(self):
        assert_raises(data.DataError, data.parse, './nojson.tgz')

    @skipif(not DATADIR)
    def test_invalid_input(self):
        assert_raises(data.DataError, data.parse, './')
        assert_raises(data.DataError, data.parse, __file__)

class test_all_readers(object):

    @skipif(not DATADIR)
    def setUp(self):
        self.readers = data.data.READERS

    @skipif(not DATADIR)
    def all_readers(object):
        # good test data must follow specific naming convention
        for filetype in self.readers.iterkeys():
            parser = data.get_handler(filetype)
...
class test_write(object):
    ...
    def setUp(self):
        ...
        self.ds = data.parse(testdata, load_data=True)

    @skipif(not DATADIR)
    def test_nifti_write(self):
        with tempfile.TemporaryDirectory() as tempdir:
            outbase = os.path.join(tempdir, 'trashme')
            data.write(self.ds, self.ds.data, filetype='nifti', outbase=outbase)
            print glob.glob(outbase + '*')
            assert (len(glob.glob(outbase + '*')) >= 1)

    @skipif(not DATADIR)
    def test_no_filetype(self):
        assert_raises(data.DataError, data.write, self.ds, self.ds.data, filetype=None, outbase='trashme')

    @skipif(not DATADIR)
    def test_empty_meta(self):
        assert_raises(data.DataError, data.write, None, self.ds.data, filetype='nifti', outbase='trashme')

    @skipif(not DATADIR)
    def test_empty_data(self):
        assert_raises(data.DataError, data.write, self.ds, None, filetype='nifti', outbase='trashme')
